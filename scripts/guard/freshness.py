...
def check_skill_load_gate(
    ...
    if not marker or not marker.exists() or not marker.is_file():
        if mandatory:
            result.add_error(
                "skill load gate: SKILL.md not marked loaded for this session — load it before any target command"
            )
            result.add_info("load with: read_file path=skills/pentest/SKILL.md")
            result.add_info(
                'then run: python scripts/violin_guard.py check-skill-loaded --eng-dir "$ENG_DIR" --session-id "<session label>"'
            )
        else:
            result.add_warning(
    ...
    for hid, body in entries:
        ...
        if status in {"Validated", "Rejected"}:
            if ts is None:
                ...
                result.add_warning(
                    f"hypothesis {hid} ({status}) last updated {_age_hours(ts):.0f}h ago (>{max_age_hours}h)"
                )
                stale_entries += 1
        if status == "Candidate":
            ...
            if linked and linked.upper().startswith("FIND-"):
                result.add_warning(
                    f"hypothesis {hid} is Candidate but already links {linked} — promote to Validated/Rejected"
                )
    ...
    if phase in {"REPORTING", "RETROSPECTIVE"}:
        ...
        if resolved:
            ...
            if not re.search(r"H-\d+:", tail):
                result.add_warning(
                    "hypotheses.md 'Resolved Theories' is empty at reporting time — record validated/rejected theories"
                )
    return result
...
def check_findings_freshness(eng_dir: Path, phase: str) -> CheckResult:
    ...
    if not found:
        result.add_warning(
            "no findings file found (e.g. evidence/vuln-research/findings.md) — record findings as they emerge"
        )
        return result
    if all(p.read_text(encoding="utf-8", errors="replace").strip() == "" for p in found):
        result.add_warning(
            "findings file exists but is empty — populate it as findings are validated"
        )
    return result
