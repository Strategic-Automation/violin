...
- **Use the guard** — Before every target-touching terminal command, run `check-command` **with `--eng-dir`, `--target`, and `--skill-loaded-file`** (these activate explicit target authorization plus the skill-load, PTT, history, and hypothesis freshness guards): `python $HOME/.hermes/profiles/violin/scripts/violin_guard.py check-command --scope $ENG_DIR/scope/scope.yaml --eng-dir "$ENG_DIR" --target "<primary-host-or-ip>" --phase <PHASE> --command "<cmd>" --skill-loaded-file "$ENG_DIR/state/.skill-loaded-<session-id>"`. Exit code `1` blocks execution; exit code `2` requires explicit review or approval. Omitting `--eng-dir` disables the freshness guards and leaves the engagement untracked — never do that.
...
```text
python $HOME/.hermes/profiles/violin/scripts/violin_guard.py check-command --scope $ENG_DIR/scope/scope.yaml --eng-dir "$ENG_DIR" --target "<primary-host-or-ip>" --phase <PHASE> --command "<cmd>" --skill-loaded-file "$ENG_DIR/state/.skill-loaded-<session-id>"
```
