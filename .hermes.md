...
python $HOME/.hermes/profiles/violin/scripts/violin_guard.py check-skill-loaded --eng-dir "$ENG_DIR" --session-id "<session-id>"
```
...
- **Run pentest tools** through `violin_exec`, `violin_exec_burst`, or the typed guard adapters. Raw `terminal` is limited to host-local preparation and administration.
...
- **Hermes plugin:** `plugins/violin_guard/` registers typed guard tools (`violin_check_command`, `violin_record_ptt`, `violin_record_hypothesis`, `violin_exec`, `violin_sync_done`, and adapters) that call the shared service directly. The plugin is required for target execution.
